...
import os, subprocess, sqlite3, platform, re, requests, logging
...
if platform.system() == 'Darwin':
    ...
    pkg = re.findall("commandlinetools-mac-[\d]{7}_latest\.zip",requests.get('https://developer.android.com/studio#command-tools').text)[0]
    ...
    pkg = re.findall("commandlinetools-linux-[\d]{7}_latest\.zip",requests.get('https://developer.android.com/studio#command-tools').text)[0]
    tools = 'https://dl.google.com/android/repository/'+pkg
    _os = 'Linux'
elif platform.system() == 'Windows':
    pt = 'https://dl.google.com/android/repository/platform-tools-latest-windows.zip'
    pkg = re.findall("commandlinetools-win-[\d]{7}_latest\.zip",requests.get('https://developer.android.com/studio#command-tools').text)[0]
    tools = 'https://dl.google.com/android/repository/'+pkg
    r = requests.get('https://api.github.com/repos/intel/haxm/releases/latest').json()
...
def ihv():
    if _os == 'Darwin':
        ...
        if Path(sdkenv+'/haxm').exists() == True:
            try:
                ...
                print(colored('haxm is now installed','green'))
        elif Path(sdkenv+'/gvm').exists() == True:
            try:
...
def isdku():
    ...
    with open(str(Path.home()/'.android/repositories.cfg'), mode='a'): pass
    if _os == 'Darwin' or _os == 'Linux':
    ...
    if Path(sdkenv+'/cmdline-tools/emulator').exists() == False:
...
def isdk():
    ...
    if request.method == 'POST':
        ...
        for id in selection:
                cur.execute('UPDATE sdk SET installed = 1 WHERE id ='+str(id))
                con.commit()
                my_image = cur.execute('SELECT image FROM sdk WHERE id ='+str(id))
                for row in my_image:
...
def usdk():
    ...
    if request.method == 'POST':
        selection = request.form.getlist('checkbox')
        for id in selection:
                cur.execute('UPDATE sdk SET installed = 0 WHERE id ='+str(id))
                con.commit()
                my_image = cur.execute('SELECT image FROM sdk WHERE id ='+str(id))
                for row in my_image:
